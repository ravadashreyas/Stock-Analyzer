                ...
                <tbody className="divide-y divide-gray-200 dark:divide-zinc-700">
                    {portfolio.map((port) => (
                        <tr key={port.user_id} className="hover:bg-gray-50 dark:hover:bg-zinc-900">
                            <td className={`${tdClass} font-mono`}>{port.user_id}</td>
                            ...
                            <td className={`${tdClass} text-gray-500`}>{port.price_at_purchase}</td>
                            <td className={`${tdClass} text-gray-500`}>{port.current_price}</td>
                            <td className={`${tdClass} text-right`}>{port.number_of_shares}</td>
                            <td className={`${tdClass} text-right`}>{port.total_cost}</td>
